...
def get_border_vertices(buffered):
    """返回缓冲区边界顶点（去重）"""
...
def plan_safe_path(start, end, obstacles, fly_mode):
    ...
    生成航点序列，点数尽可能少。
     - 直飞最短：始终只返回起点和终点（2个点）
     - 左右绕飞：返回起点 -> 缓冲区关键顶点 -> 终点（顶点数通常少于10）
     - 弧线最短：沿缓冲区边界采样12个点（包含起点终点共14个以内）
    """
    # 直飞最短：无论有无障碍物，都只有起点终点
    if fly_mode == "直飞最短":
        return [start, end]
    ...
    # 获取缓冲区边界顶点
    vertices = get_border_vertices(buffered)
    n = len(vertices)
    ...
    # 计算顺时针（索引递增）和逆时针（索引递减）的顶点序列
    cw_indices = []
    i = e_idx
    while i != x_idx:
        cw_indices.append(i)
        i = (i + 1) % n
    cw_indices.append(x_idx)

    ccw_indices = []
    i = e_idx
    while i != x_idx:
        ccw_indices.append(i)
        i = (i - 1) % n
    ccw_indices.append(x_idx)

    # 根据飞行模式选择方向
    if fly_mode == "左侧绕飞":
        selected = ccw_indices   # 逆时针
    elif fly_mode == "右侧绕飞":
        selected = cw_indices    # 顺时针
    else:  # 弧线最短航线
        selected = cw_indices if len(cw_indices) <= len(ccw_indices) else ccw_indices

    # 收集边界点
    bypass = [vertices[i] for i in selected]

    # 弧线模式对边界点进行均匀采样（减少点数）
    if fly_mode == "弧线最短航线" and len(bypass) > 6:
        # 每隔几个点取一个，保留最多6个中间点
        step = max(1, len(bypass) // 6)
        bypass = bypass[::step]
        if bypass[-1] != vertices[selected[-1]]:
            bypass.append(vertices[selected[-1]])

    # 构建完整路径
    full_path = [start] + bypass + [end]
    return full_path
