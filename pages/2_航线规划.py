...
def catmull_rom_spline(points, num_segments=40):
    """
    对点列进行 Catmull-Rom 样条插值，返回平滑后的多段线点集。
    适用于弧线模式，使折线变成平滑曲线。
    """
    if len(points) < 2:
        return points
...
def plan_safe_path(start, end, obstacles, fly_mode):
    """
    统一绕飞逻辑：
        1. 计算直线与障碍物缓冲区的交点（入口、出口）。
        2. 获取缓冲区边界顶点序列。
        3. 根据模式选择顺时针/逆时针方向的边界路径。
        4. 左右绕飞模式直接返回折线；弧线模式对折线进行 Catmull-Rom 平滑。
    """
    # 直飞模式
    if fly_mode == "直飞最短":
        return [start, end]

    # 无任何障碍物时，弧线模式生成一条简单贝塞尔曲线（美观）
    if not obstacles:
        if fly_mode == "弧线最短航线":
    ...
    def nearest_idx(pt):
        min_d = float('inf')
        idx = 0
        for i, p in enumerate(boundary_coords):
    ...
    # 根据模式返回
    if fly_mode == "弧线最短航线":
        # 对核心路径进行 Catmull-Rom 平滑，生成高密度多段线（模拟弧线）
        return catmull_rom_spline(core_path, num_segments=50)
    else:
        # 左右绕飞折线路径
        return core_path
...
# ==================== 地图显示 ====================
st.header("🗺️ 航线规划（弧线最短已优化，平滑不穿越）")
