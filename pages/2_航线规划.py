...
# ==================== 🔥 唯一正确：真正最短弧线绕障（双切线+最短小圆弧） ====================
def get_shortest_arc_path(start, end, obstacle):
    """
    无人机/计算几何 标准最短弧线：
    起点 → 外切线点 → 安全区最短圆弧 → 外切线点 → 终点
    【绝对最短，没有更短的弧线！】
    """
    obs_poly = Polygon(obstacle['points'])
    safe_poly = get_obstacle_with_buffer(obs_poly)
    start_pt = Point(start)
    end_pt = Point(end)

    # 获取起点/终点到安全区的最近切点（关键：最短路径的唯一切点）
    p_start = nearest_points(start_pt, safe_poly.exterior)[1]
    p_end = nearest_points(end_pt, safe_poly.exterior)[1]
    center = safe_poly.centroid

    # 计算两点极角
    angle1 = math.atan2(p_start.y - center.y, p_start.x - center.x)
    angle2 = math.atan2(p_end.y - center.y, p_end.x - center.x)
    radius = safe_poly.exterior.distance(center)  # 安全区半径
...
def plan_safe_path(start, end, obstacles, fly_mode):
    """
    优化：左右绕飞为多段折线，弧线最短为【真正最短切线圆弧】
    """
    if not obstacles:
        if fly_mode == "弧线最短航线":
            num = 50
            return [(start[0] + (end[0]-start[0])*i/num, start[1] + (end[1]-start[1])*i/num) for i in range(num+1)]
        return [start, end]
    ...
    if fly_mode == "左侧绕飞":
        ...
        # ✅ 直接返回真正最短弧线，无任何多余处理
        arc_points = get_shortest_arc_path(start, end, obstacle)
