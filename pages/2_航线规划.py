...
def plan_safe_path(start, end, obstacles, fly_mode, swap_direction):
    ...
    # 根据模式选择绕行方向
    if fly_mode == "左侧绕飞":
        # 左侧绕飞：默认使用逆时针（因为逆时针沿边界时，障碍物在右侧，不符合？但我们保留交换选项）
        # 直接根据 swap_direction 决定
        if swap_direction:
            direction = 1   # 顺时针
        else:
            direction = -1  # 逆时针
