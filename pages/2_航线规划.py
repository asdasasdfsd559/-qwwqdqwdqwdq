...
# ------------------ 几何辅助 ------------------
def nearest_point_on_boundary(pt, coords):
    min_dist = float('inf')
    idx = 0
...
def plan_safe_path(start, end, obstacles, fly_mode):
    ...
    三种模式：
    - 直飞最短：直接返回两点
    - 左侧绕飞：多段折线（顺时针沿缓冲区顶点，索引递增）
    - 右侧绕飞：多段折线（逆时针沿缓冲区顶点，索引递减）
    - 弧线最短航线：自动选择较短侧，并生成平滑曲线（高密度多段线）
    """
    # 无障碍物情况
    if not obstacles:
        if fly_mode == "弧线最短航线":
    ...
    # 根据模式确定方向
    if fly_mode == "左侧绕飞":
        use_clockwise = True        # 左侧绕飞 = 顺时针
    elif fly_mode == "右侧绕飞":
        use_clockwise = False       # 右侧绕飞 = 逆时针
    else:  # 弧线最短航线
        use_clockwise = cw_steps <= ccw_steps   # 选择更短侧
