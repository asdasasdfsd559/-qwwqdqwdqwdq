...
with st.sidebar:
    ...
    st.subheader("📍 起点A")
    a_lat = st.number_input("纬度", value=st.session_state.a_lat, format="%.6f")
    a_lng = st.number_input("经度", value=st.session_state.a_lng, format="%.6f")
    if st.button("设置A点"):
    ...
    st.subheader("📍 终点B")
    b_lat = st.number_input("纬度", value=st.session_state.b_lat, format="%.6f")
    b_lng = st.number_input("经度", value=st.session_state.b_lng, format="%.6f")
    if st.button("设置B点"):
    ...
    obs_height = st.number_input("高度(m)", 1, 500, 30)
    if st.button("保存障碍物"):
